  ...
  try {
      for (const auto& name : joint_names) {
        if (name.find("pivot") != std::string::npos) {
            pivot_joints_.push_back(hw->getHandle(name)); // 舵向关节（pivot）握手
...
void SentryChassisController::update(const ros::Time& time, const ros::Duration& period) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (!all_wheels_aligned) {
        if (std::abs(wheel_vel_fdb) > 0.2){
        current_target = last_target_velocities_[i]; 
        }// 保持速度
        else{
            current_target = 0.0;
    ...
    if (ctrl_mode_ == CHASSIS_STOP) {
        ...
        // 判断：是否处于高负载旋转状态？
        // 依据：看平滑后的角速度指令是否 > 0.1 (按Q键或小陀螺时，这个值会很大)
        bool is_rotating_heavy_load = std::abs(ramped_vel_.angular.z) > 0.1;

        if (is_rotating_heavy_load) {
            // a. 静态摩擦补偿 (源自 URDF friction="0.05")
            // 只要想动，先给个过路费，消除死区震荡
            if (std::abs(current_target) > 0.01) {
                double static_friction = 0.05; 
                ff_effort += (current_target > 0 ? 1.0 : -1.0) * static_friction;
            }

            // b. 速度前馈 (KV)
            // 源自你的PID图表：0.5Nm / 4.65rad/s ≈ 0.11
            double kv_spin = 0.11; 
            ff_effort += current_target * kv_spin;
        }
        // 如果只是平移 (W/A/S/D)，is_rotating_heavy_load 为 false，ff_effort 保持 0.0
        // 这样绝对不会破坏平移的手感

        // 计算 PID (P项建议适当减小，因为主力已经由前馈提供了)
